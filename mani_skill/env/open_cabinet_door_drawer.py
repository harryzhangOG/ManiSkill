...
class OpenCabinetEnvBase(BaseEnv):
    ...
    def _find_handles_from_articulation(self):
        ...
        for link in self.cabinet.get_links():
            ...
            for visual_body in link.get_visual_bodies():
                if 'knob' in visual_body.get_name():
                    continue
                if 'door' not in visual_body.get_name() and 'handle' not in visual_body.get_name():
                    continue
                handles_visual_body_ids[link_name].append(
                    visual_body.get_visual_id())
                for i in visual_body.get_render_shapes():
                    vertices = apply_pose_to_points(
                        i.mesh.vertices * i.scale, i.pose)
                    mesh = np2mesh(vertices, i.mesh.indices.reshape(-1, 3))
                    o3d_info[link_name].append(mesh)
                    handles_info[link_name].append(
                        (i.mesh.vertices * i.scale, i.mesh.indices, i.pose))
        ...
        for link in self.cabinet.get_links():
            ...
            def crop_by_box():
                sign = np.logical_and(
                    pcd_world >= center - box_size, pcd_world <= center + box_size)
    ...
    def _choose_target_link(self, joint_type):
        ...
        if self.fixed_target_link_id is not None:
            ...
            self.target_index = self._level_rng.choice(
                len(joints))  # only sample revolute/prismatic joints
        ...
        self.target_index_in_active_joints = self.cabinet.get_active_joints().index(
            self.target_joint)
    ...
    def _place_robot(self):
        ...
        # base pos
        # to make the gripper closer to the cabinet
        ...
        # base orientation
        perturb_orientation = self._level_rng.uniform(
    ...
    def _set_joint_physical_parameters(self):
        for joint in self.cabinet.get_joints():
            if self.joint_friction is not None:
                joint.set_friction(sample_from_tuple_or_scalar(
                    self._level_rng, self.joint_friction))
            if self.joint_damping is not None:
                joint.set_drive_property(stiffness=0,
                                         damping=sample_from_tuple_or_scalar(
                                             self._level_rng, self.joint_damping),
    ...
    def compute_other_flag_dict(self):
        ...
        dist_ee_to_handle = np.linalg.norm(
            ee_to_handle, axis=-1).min(-1).min(-1)  # [2]
        ...
        dist_ee_mid_to_handle = trimesh.proximity.ProximityQuery(
            handle_mesh).signed_distance(ee_cords.mean(0)).max()
    ...
    def compute_dense_reward(self, action, state=None):
        ...
        target_pose = self.target_link.get_pose(
        ) * self.grasp_pose[self.target_link_name][0]
        target_pose_2 = self.target_link.get_pose(
        ) * self.grasp_pose[self.target_link_name][1]
        ...
        cabinet_vel = self.cabinet.get_qvel(
        )[self.target_index_in_active_joints]
        ...
        rew_ee_mid_handle = normalize_and_clip_in_interval(
            dist_ee_mid_to_handle, -0.01, 4E-3) - 1

        reward = gripper_angle_rew + rew_ee_handle + \
            rew_ee_mid_handle - (dist_coefficient + vel_coefficient)
        ...
        if other_info['ee_close_to_handle']:
            stage_reward += 0.5
            vel_reward = normalize_and_clip_in_interval(
                cabinet_vel, -0.1, 0.5) * vel_coefficient  # Push vel to positive
    ...
    def _post_process_view(self, view_dict):
        ...
        masks = [np.zeros(visual_id_seg.shape, dtype=np.bool)
                 for _ in range(3)]
