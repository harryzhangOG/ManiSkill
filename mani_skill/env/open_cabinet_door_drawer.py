...
class OpenCabinetEnvBase(BaseEnv):
    ...
    def _find_handles_from_articulation(self):
        ...
        for link in self.cabinet.get_links():
            ...
            for visual_body in link.get_visual_bodies():
                if (
                    "knob" in visual_body.get_name()
                    or "movable_part" in visual_body.get_name()
                ):
                    continue
                kw_in = False
                for kw in door_kw:
    ...
    def _choose_target_link(self, joint_type):
        ...
        if self.fixed_target_link_id is not None:
            ...
            self.target_index = self._level_rng.choice(
                len(joints)
            )  # only sample revolute/prismatic joints
    ...
    def _prepare_for_obs(self):
        self.handle_visual_ids = self.handles_visual_body_ids[
            self.target_link.get_name()
        ]
    ...
    def _set_joint_physical_parameters(self):
        for joint in self.cabinet.get_joints():
            if self.joint_friction is not None:
                joint.set_friction(
                    sample_from_tuple_or_scalar(self._level_rng, self.joint_friction)
                )
    ...
    def compute_other_flag_dict(self):
        ...
        handle_mesh = trimesh.Trimesh(
            vertices=apply_pose_to_points(
                np.asarray(self.o3d_info[self.target_link_name][-2].vertices),
                self.target_link.get_pose(),
            ),
            faces=np.asarray(
                np.asarray(self.o3d_info[self.target_link_name][-2].triangles)
            ),
        )
    ...
    def compute_eval_flag_dict(self):
        flag_dict = {
            ...
            "open_enough": self.cabinet.get_qpos()[self.target_index_in_active_joints]
            >= self.target_qpos,
        }
    ...
    def compute_dense_reward(self, action, state=None):
        ...
        target_pose = (
            self.target_link.get_pose() * self.grasp_pose[self.target_link_name][0]
        )
        target_pose_2 = (
            self.target_link.get_pose() * self.grasp_pose[self.target_link_name][1]
        )
        ...
        if other_info["ee_close_to_handle"]:
            ...
            dist_reward = (
                normalize_and_clip_in_interval(
                    self.cabinet.get_qpos()[self.target_index_in_active_joints],
                    0,
                    self.target_qpos,
                )
                * dist_coefficient
...
class OpenCabinetDoorEnv(OpenCabinetEnvBase):
    def __init__(self, *args, split="train", **kwargs):
        super().__init__(
            f"../assets/config_files/open_cabinet_door.yml", *args, **kwargs
        )
...
class OpenCabinetDrawerEnv(OpenCabinetEnvBase):
    def __init__(
        self,
        yaml_file_path=f"../assets/config_files/open_cabinet_drawer.yml",
        *args,
        **kwargs,
    ):
