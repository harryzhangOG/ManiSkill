...
class CombinedAgent:
    def __init__(self, agents):
        ...
        self.action_indices = np.concatenate(
            ([0], np.cumsum(self.action_dims)))
    ...
    def set_state(self, state, by_dict=False):
        if by_dict:
            ...
            for i, agent in enumerate(self.agents):
                agent.set_state(
                    state[self.state_indices[i]:self.state_indices[i+1]], by_dict=False)
...
class Agent:
    def __init__(self, engine: Engine, scene: Scene, config):
        ...
        for name in self.config['controllable_joints']:
            ...
            if action_type == 'velocity':
                ...
                raise RuntimeError(
                    'Only velocity or position are valid action types')
            controllers.append(controller)

        self.controllers = controllers
        all_action_range = np.array(all_action_range, dtype=np.float32)
        self._action_range = spaces.Box(
            all_action_range[:, 0], all_action_range[:, 1])
        self.num_ee = None
                ...
                if type(controller) == PositionController:
                    ...
                    if n:
                        controller_state.append(
                            controller.velocity_pid._prev_err)
                    else:
    ...
    def set_state(self, state, by_dict=False):
        if not by_dict:
            ...
            # remove fingers_pos and fingers_vel
            state = state[self.num_ee*12:]
            state_dict = {
...
class DummyMobileAgent(Agent):
    ...
    def get_state(self, by_dict=False, with_controller_state=True):
        state_dict = super().get_state(
            by_dict=True, with_controller_state=with_controller_state)
...
class DummyMobileAdjustableHeightAgent(DummyMobileAgent):
    def __init__(self, *args, **kwargs):
        ...
        body = [link for link in self.robot.get_links() if link.name ==
                'adjustable_body'][0]
...
class MobileA2DualArmAgent(DummyMobileAdjustableHeightAgent):
    def __init__(self, *args, **kwargs):
        ...
        self.num_ee = 2
        self.full_state_len = len(self.get_state(
            by_dict=False, with_controller_state=True))
